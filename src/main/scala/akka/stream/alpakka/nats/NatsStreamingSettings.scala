...
object DeliveryStartPosition {
  ...
  final def fromConfig(config: Config): DeliveryStartPosition =
    Try(config.getLong("deliver-after-sequence-number")) match {
      case Success(seq) => AfterSequenceNumber(seq)
      case Failure(_) =>
        Try(Instant.ofEpochMilli(config.getLong("deliver-after-epoch-millis")))
          .orElse(
            Try(Instant.parse(config.getString("deliver-after-instance")))
          ) match {
          case Success(time) => AfterTime(time)
          case Failure(_) =>
            config.getString("deliver").toLowerCase match {
...
case object NatsStreamingConnectionSettings {
  private final def url(host: String, port: Int): String =
    "nats://" + host + ":" + port.toString
...
sealed trait NatsStreamingSubscriptionSettings {
  ...
  def subscriptionOptions: SubscriptionOptions = {
    ...
    val bAckWait =
      autoRequeueTimeout.map(bMaxInFlight.ackWait).getOrElse(bMaxInFlight)
    val bDurable =
      durableSubscriptionName.map(bAckWait.durableName).getOrElse(bAckWait)
    ...
    startPosition match {
      ...
      case DeliveryStartPosition.AfterSequenceNumber(seq) =>
