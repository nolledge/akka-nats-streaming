...
case class IncomingMessage[T](data: T, subject: Option[String] = None)
    extends NatsStreamingIncoming[T] {
  def transform[T2](f: T => T2): IncomingMessage[T2] =
    IncomingMessage(f(data), subject)
}

case class IncomingMessageWithAck[T](data: T, subject: Option[String] = None)
    extends NatsStreamingIncoming[T] {
  ...
  def ack: Try[T] =
    if (promise.trySuccess(Done)) Success(data)
    else Failure(new Exception("Already completed"))
}
...
case class OutgoingMessage[T](data: T, subject: Option[String] = None)
    extends NatsStreamingOutgoing[T] {
  def transform[T2](f: T => T2): OutgoingMessage[T2] =
    OutgoingMessage(f(data), subject)
}

case class OutgoingMessageWithCompletion[T](
    data: T,
